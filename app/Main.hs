...
[covered, pressed, flagged, _qmark, _qmarkpressed, mine, blownup, _xmine ] = 
    take 8 $ map (\x -> Rectangle (P (V2 (14 + x) 195)) (V2 15 15)) [0,17..]
...
dispatch :: Renderer -> [EventPayload] -> Hrm ()
dispatch renderer ((KeyboardEvent ke):_)
  | keyboardEventKeyMotion ke == Pressed
    && keysymKeycode (keyboardEventKeysym ke) == KeycodeQ =
        liftIO $ putStrLn "quit!"
dispatch renderer ((MouseButtonEvent me):xs)
  | mouseButtonEventMotion me == Pressed
    && i >= 0 && i <= 9 && j >= 0 && j <= 9 = do
      ...
      dispatch renderer xs
      where P (V2 x y) = mouseButtonEventPos me
            i = CInt $ x `div` 60
            j = CInt $ y `div` 60
            mb = mouseButtonEventButton me
dispatch renderer (_:xs) = dispatch renderer xs
dispatch renderer [] = appLoop renderer

houseKeeping :: Grid -> (Bool, Grid)
houseKeeping grid
  | lost = (True, grid // 
      [(ix, Tile Mine Shown) | ix <- indices grid, isMine (grid ! ix)])
  | otherwise = (False, grid)
  where lost = any (\(Tile ti tv) -> ti == BlownUp) $ elems grid
...
appLoop renderer = do
  ...
  present renderer
  if lost
     then liftIO $ threadDelay 2000000
     else dispatch renderer $ map eventPayload events
...
makeGrid g = array ((0, 0), (9, 9)) [((i, j), choose (i, j)) | i <- [0..9], j <- [0..9]]
    where choose c
          ...
          clamp' = clamp 0 9
          neighbors (i, j) = 
              sum [ if Set.member (clamp' i+oi, clamp' j+oj) mines
                       then 1 else 0
                    | oi <- [-1..1], oj <- [-1..1] ]
